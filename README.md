# CodeX IDE

[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg?style=flat-square)](LICENSE)
[![Version: 1.0.0](https://img.shields.io/badge/Version-1.0.0-emerald.svg?style=flat-square)](https://github.com/Suryanshu-Nabheet/CodeX)
[![Platform: Cross-Platform](https://img.shields.io/badge/Platform-macOS%20%7C%20Windows%20%7C%20Linux-lightgrey.svg?style=flat-square)](#)

**This** is a Basic Version AI-powered IDE built for modern developers. Featuring the sophisticated **Oxocarbon** theme and an offline-first architecture, CodeX delivers a professional, distraction-free environment for high-performance development.
**Official CodeBase of CodeX built on top of vscode is here [Code](https://github.com/Suryanshu-Nabheet/CodeX).**

---

## Features

### Intelligence First

-   **AI Code Completion** – Agentic inline ghost text generated by the configured AI provider
-   **Native AI Assistance** – Seamlessly integrated AI for code generation, debugging, and complex refactoring
-   **Deep Language Intelligence** – Powered by advanced Language Server Protocol (LSP) for precise completions and navigation
-   **Context-Aware Chat** – Chat with your codebase directly within the editor

### Elite Aesthetics

-   **7 Premium Themes** – Choose from CodeX Dark, Monokai, Dracula, GitHub Dark, Solarized Dark, Nord, and One Dark
-   **Instant Theme Switching** – Change themes on the fly without restarting
-   **Customizable Appearance** – Font family, size, and editor preferences
-   **Micro-Animation Engine** – Every interaction is smooth, intentional, and visually rewarding
-   **Uncluttered Interface** – Designed for maximum real estate, focusing entirely on your code

### Extension Ecosystem

-   **OpenVSX Marketplace** – Access thousands of extensions from the open-source marketplace
-   **One-Click Installation** – Browse, install, and manage extensions seamlessly
-   **Extension Management** – View installed extensions, ratings, and details
-   **Theme Extensions** – Install community themes to personalize your experience

### Professional Tooling

-   **Multimodal Terminal** – High-performance, integrated multi-tab terminal with full shell support
-   **Unified Sidebars** – Elegant search, file tree, extensions, and AI panels that respect your visual focus
-   **Enterprise-Grade Stability** – Built on a hardened Electron architecture for reliable daily use

### Privacy & Performance

-   **Offline-First** – Your core workflow resides on your machine. No mandatory cloud dependencies
-   **Ultra-Fast Performance** – Optimized file indexing and rendering for projects of any size

---

## Quick Start

### Prerequisites

-   **Node.js** v18+
-   **npm** v9+
-   **Git**

### Installation

```bash
# Clone the repository
git clone https://github.com/Suryanshu-Nabheet/CodeX.git
cd CodeX

# Run setup
./scripts/setup.sh       # macOS/Linux
.\scripts\setup.ps1      # Windows
# or: npm run setup / npm run setup:win

# Launch CodeX
npm start
```

---

## Keyboard Shortcuts

| Action              | Shortcut               |
| :------------------ | :--------------------- |
| **Quick Open**      | `Cmd/Ctrl + P`         |
| **Command Palette** | `Cmd/Ctrl + Shift + P` |
| **Global Search**   | `Cmd/Ctrl + Shift + F` |
| **Toggle Terminal** | `` Ctrl + ` ``         |
| **Save All**        | `Cmd/Ctrl + Alt + S`   |
| **AI Command Bar**  | `Cmd/Ctrl + K`         |
| **Manual Inline Completion** | `Cmd/Ctrl + Shift + Space` or `Alt + \` |

---

## Technical Architecture

CodeX is built on a modern, high-performance stack:

-   **Core Orchestrator** – [Electron](https://www.electronjs.org/)
-   **Visual Interface** – [React 18](https://reactjs.org/) & [Tailwind CSS](https://tailwindcss.com/)
-   **State Management** – [Redux Toolkit](https://redux-toolkit.js.org/)
-   **Editor Engine** – [CodeMirror 6](https://codemirror.net/)
-   **Type System** – [TypeScript](https://www.typescriptlang.org/)

---

## AI Code Completion

CodeX 1.0.0 uses an AI-first completion stack:

-   **AI inline completion** appears as ghost text while typing and uses provider, editor, diagnostic, and open-file context.
-   **LSP support** starts automatically for supported languages and remains focused on language intelligence such as diagnostics, hover, symbols, and definitions.

AI completion uses the provider selected in Settings. Ollama works as the local-first default at `http://localhost:11434`; cloud providers require the corresponding API key in Settings. See [AI completion docs](docs/completion.md) for setup and troubleshooting.

---

## Language Support

CodeX provides full intelligence for:

-   **Web** – JavaScript, TypeScript, HTML, CSS/SCSS, JSON
-   **Systems** – C, C++, Rust, Go
-   **Backend** – Python, Java, PHP, Ruby
-   **Data** – SQL, YAML, TOML, Markdown

---

## Contributing

We believe in the power of open-source excellence. To contribute to CodeX, please review our [Contributing Guidelines](CONTRIBUTING.md).

---

## License

CodeX is released under the **MIT License**. See the [LICENSE](LICENSE) file for details.

---

## Developed By

**Suryanshu Nabheet**  
_Lead Architect & Designer_

[GitHub](https://github.com/Suryanshu-Nabheet) • [LinkedIn](https://linkedin.com/in/suryanshu-nabheet)

---

<div align="center">

**CodeX IDE**  
_Engineered for the Modern Developer_

</div>



